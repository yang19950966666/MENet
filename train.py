...
def train(train_loader, model, optimizer, epoch):
    ...
    if (epoch + 1) % 5 == 0 or (epoch + 1) == opt.epoch:
        torch.save(model.state_dict(), save_path + 'MENet-%d.pth' % epoch)
        print('[Saving Snapshot:]', save_path + 'MENet-%d.pth' % epoch)
        file.write('[Saving Snapshot:]' + save_path + 'MENet-%d.pth' % epoch + '\n')

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--epoch', type=int,
                        default=25, help='epoch number')   #25
    parser.add_argument('--lr', type=float,
                        default=1e-4, help='learning rate')
    parser.add_argument('--batchsize', type=int,
                        default=8, help='training batch size') #8*520*520 = xknet.best
    parser.add_argument('--trainsize', type=int,
                        default=520, help='training dataset size')
